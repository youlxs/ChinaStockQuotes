...
namespace PetaPoco
{
    ...
    public partial class Database : IDisposable
    {
        ...
        public class PocoColumn
        {
            ...
            public virtual object GetValueWithAccessor(object target) { return DynamicPropertyDecorator.GetAccessor(PropertyInfo).GetValue(target); }
        }
    ...
    public partial class Database
    {
        ...
        static Dictionary<string, DataTable> DataTableMappers = new Dictionary<string, DataTable>();

        public void BulkInsertRecords<T>(IEnumerable<T> collection, string tableName, bool autoIncrement)
        {
            try
            {
                ////sw.Start();
                ...
                using (var cmd = CreateCommand(_sharedConnection))
                {
                    ...
                    var dt = GetDataTableInfo(tableName);
                    var map = new Dictionary<string, PocoColumn>();
                    ...
                    foreach (var col in pd.Columns)
                    {
                        ...
                        map.Add(col.Value.ColumnName.ToLower(), col.Value);
                    }
                    ...
                    foreach (var poco in collection)
                    {
                        ...
                        {
                            var col = map[dt.Columns[i].ColumnName.ToLower()];
                            ...
                            dr[dt.Columns[i]] = AddParam(col.GetValueWithAccessor(poco));
                        }

                        dt.Rows.Add(dr);
                    }

                    cmd.BatchSize = collection.Count();
            ...
            finally
            {
                CloseSharedConnection();
                ////this.sw.Stop();
                ////System.Diagnostics.Debug.WriteLine(string.Format("Type {0} count {1} totalcost {2}", typeof(T), collection.Count(), sw.ElapsedMilliseconds));
                ////this.sw.Reset();
            }
        }

        public SqlBulkCopy CreateCommand(IDbConnection connection)
        {
            if (_transaction == null)
                return new SqlBulkCopy((SqlConnection)connection);
            return new SqlBulkCopy((SqlConnection)connection, SqlBulkCopyOptions.Default, (SqlTransaction)_transaction);
        }
        ...
        DataTable GetDataTableInfo(string tableName)
        {
            ...
            {
                DataTable table;
                if (DataTableMappers.TryGetValue(tableName, out table))
                    return table.Copy();
            }
            finally
            {
            ...
            try
            {
                ...
                using (var cmd = CreateCommand(_sharedConnection, string.Format("select * from {0} where 1 = 2", tableName), null))
                {
                    try
                    {
                        ...
                        DataTableMappers.Add(tableName, table);
    ...
    public static class DynamicPropertyDecorator
    {
        private static object m_mutex = new object();

        private static Dictionary<PropertyInfo, DynamicPropertyAccessor> m_cacheAccessor =
            new Dictionary<PropertyInfo, DynamicPropertyAccessor>();

        public static DynamicPropertyAccessor GetAccessor(PropertyInfo propertyInfo)
        {
            DynamicPropertyAccessor accessor;

            if (!m_cacheAccessor.TryGetValue(propertyInfo, out accessor))
            {
                lock (m_mutex)
                {
                    accessor = new DynamicPropertyAccessor(propertyInfo);
                    m_cacheAccessor[propertyInfo] = accessor;
                }
